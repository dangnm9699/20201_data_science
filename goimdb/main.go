...
func main() {
		...
		go func(idx int) {
			for count := idx * req; count < (idx+1)*req; count++ {
				...
				c.OnHTML("div#content-2-wide", func(e *colly.HTMLElement) {
					...
					movie.Genres = genres
					// get Duration
					runtime := e.ChildText("div#main_top > div.title-overview > div#title-overview-widget > div.vital > div.title_block > div.title_bar_wrapper > div.titleBar > div.title_wrapper > div.subtext > time")
					movie.Runtime = strings.TrimSpace(runtime)
					// get Budget
					budget := e.ChildText("div#main_bottom > div#titleDetails > div.txt-block:contains('Budget:')")
					movie.Budget = getMoney(budget)
					// get Cumulative
					gross := e.ChildText("div#main_bottom > div#titleDetails > div.txt-block:contains('Cumulative Worldwide Gross:')")
					movie.Gross = getMoney(gross)
					// get Director
					var director string
	...
	wg.Wait()
	log.Println("=======================================================")
	log.Println("[INF] Done, wait for 10 seconds")
